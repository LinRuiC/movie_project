...
def admin_login_req(f):
    ...
    def decorated_function(*args,**kwargs):
        if 'admin' not in session:
            return redirect(url_for('admin_blueprint.login',next=request.url))
...
def pwd():
    ...
    if form.validate_on_submit():
        ...
        redirect(url_for('admin_blueprint.logout'))
...
def tag_add():
    ...
    if formtag.validate_on_submit():
        ...
        db.session.commit()
        redirect(url_for('admin_blueprint.tag_add'))
    return render_template('admin/tag_add.html',form=formtag)
...
def tag_edit(id=None):
    ...
    if form.validate_on_submit():
        ...
        db.session.commit()
        flash('修改标签成功！','ok')
        redirect(url_for('admin_blueprint.tag_edit',id=id))
...
def movie_del(id=None):
    ...
    # 删除电影文件和封面
    os.remove(app.config['MV_DIR'] + movie.url)
    os.remove(app.config['MV_DIR'] + movie.logo)
...
def movie_edit(id=None):
    ...
    if form.validate_on_submit():
        ...
        if not os.path.exists(app.config['MV_DIR']):
            os.makedirs(app.config['MV_DIR'])
            os.chmod(app.config['MV_DIR'],'rw')

        if form.url.data.filename != '':
            file_url=secure_filename(form.url.data.filename)
            movie.url=change_filename(file_url)
            form.url.data.save(app.config['MV_DIR']+movie.url)

        if form.logo.data.filename != '':
            file_logo = secure_filename(form.logo.data.filename)
            movie.logo = change_filename(file_logo)
            form.logo.data.save(app.config['MV_DIR'] + movie.logo)
...
def preview_add():
    ...
    if form.validate_on_submit():
        ...
        if not os.path.exists(app.config['PR_DIR']):
            ...
            os.chmod(app.config['PR_DIR'],stat.S_IRWXU)
        logo=change_filename(file_logo)
...
def preview_del(id=None):
    ...
    # 删除预告图片
    os.remove(app.config['PR_DIR'] + preview.logo)
...
def comment_del(id=None):
    ...
    db.session.commit()
    flash('删除评论成功！','ok')
    return redirect(url_for('admin_blueprint.comment_list',page=1))
...
def moviecol_del(id=None):
    ...
    db.session.commit()
    flash('删除收藏成功!','ok')
    return redirect(url_for('admin_blueprint.moviecol_list',page=1))
...
def role_edit(id=None):
    ...
    if request.method == "GET":
        auths = role.auths
        auths_list=auths.split(",")
        if len(auths_list) >1 :
            form.auths.data = list(map(lambda v: int(v), auths_list))
        else:
            form.auths.data=None
...
def role_del(id=None):
    ...
    db.session.commit()
    flash("删除角色成功！", "ok")
    return redirect(url_for('admin_blueprint.role_list', page=1))
...
def auth_del(id=None):
    ...
    db.session.commit()
    flash("删除标签成功！", "ok")
    return redirect(url_for('admin_blueprint.auth_list', page=1))
...
def auth_edit(id=None):
    ...
    if form.validate_on_submit():
        ...
        db.session.commit()
        flash("修改权限成功！", "ok")
        redirect(url_for('admin_bluprint.auth_edit', id=id))
